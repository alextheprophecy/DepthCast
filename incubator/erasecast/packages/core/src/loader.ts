...
async function toBitmap(input: Exclude<ImageSource, ImageData>): Promise<ImageBitmap> {
...
export async function loadImage(input: ImageSource, maxResolution = 2048): Promise<LoadedImage> {
  ...
  if (typeof ImageData !== 'undefined' && input instanceof ImageData) {
    ...
    const bmp = await toBitmap(input as Exclude<ImageSource, ImageData>)
    bmpW = bmp.width
    bmpH = bmp.height
